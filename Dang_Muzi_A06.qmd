...
paste("The largest win spread is", spread[max_index], ", when the Bears scored", bears[max_index], "points and the Opponent scored", opponent[max_index], "points.")
```
...
```{r}
new_score <- matrix(c(37,9,17,17), nrow=2)
colnames(new_score) <- c("bears", "opponent")
game_score <- rbind(game_score, new_score)
print(game_score)
```
...
```{r}
win <- c(game_score[, "bears"]> game_score[, "opponent"])
print(win)
```
...
```{r}
number_of_factors <- function(int_input){
  count <- 0
  for (i in 1:as.integer(int_input)){
    if (as.integer(int_input) %% i == 0){
      count <- count + 1
    }
  }
  return(count)
...
```{r}
triangle <- function(length = 48, width = 48){
  area_in = length * width
  area_ft = area_in / (12*12)
  return(c(area_in, area_ft))
}

ex_tri <- triangle(102, 60)
print(paste("The area in square inches is" , ex_tri[1]))
print(paste("The area in square feet is" , ex_tri[2]))
...
```{r}
most_letter <- function(string_vec){
  len_word <- c()
  filtered <- c()
  for (word in string_vec){
    len_word <- append(len_word, nchar(word))
  }
  for (word in string_vec){
    if (nchar(word) == max(len_word)){
...
```{r}
value_check <- function(value, vector){
  checker <- FALSE
  for (word in vector){
    if (value == word){
      checker <- TRUE
